        ...
        return new Promise((resolve, reject) => {
            ...
            if (transaction.type === TYPES.ASSET_EXCHANGE) {
                assets_1.assetExchangeApi.getAsset(transaction.attachment.asset).then((asset) => {
                    const quantity = asset.decimals > 0 ? (+transaction.attachment.quantityQNT / Math.pow(10, asset.decimals)) : +transaction.attachment.quantityQNT;
                    let amount = 0;
                    if (transaction.attachment.priceNQT && transaction.attachment.priceNQT.length) {
                        amount = utils_1.utils.toEightDecimals(asset.decimals > 0 ? (+transaction.attachment.priceNQT / Math.pow(10, asset.decimals)) : +transaction.attachment.priceNQT);
                    }
